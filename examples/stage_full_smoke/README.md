...
配置模板为 `configs/eval/progress_test_remote.yaml`。先将其中的 `sampling.paths` 改为本地 Hugging Face Dataset
...
所有运行产物位于 `evaluation_output/progress-test-remote/`。该目录是中间产物，已由 `.gitignore` 忽略。
