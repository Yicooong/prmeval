...
Preserve the `bench.record.v1` `EvaluationRecord` contract across sampled and inferred artifacts. New datasets, samplers, infer baselines, and metrics should use the registries in `prmeval/core/registry.py`. Each built-in baseline owns its checkpoint, provider, or HTTP execution logic. The framework does not distinguish local and remote execution; do not revive mode/transport dispatch, adapters, or public batch APIs. Dataset-unification converters should use the registry in `dataset_unify/registry.py` and produce the fixed schema through `build_standard_dataset()` rather than adding source-specific fields to PRMEval.
