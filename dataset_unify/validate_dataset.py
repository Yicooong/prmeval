...
def validate_dataset_fields_and_types(dataset: Dataset, sample_size: int = 10) -> dict[str, Any]:
    ...
    unexpected_fields = sorted(set(dataset.features) - set(expected_fields))
    if unexpected_fields:
        validation_results["valid"] = False
        validation_results["errors"].append(f"Unexpected fields: {', '.join(unexpected_fields)}")
    ...
    for idx in sample_indices:
        ...
        try:
            ...
            partial_success = trajectory.get("partial_success")
            if partial_success is not None:
                if not isinstance(partial_success, (int, float, np.floating, np.integer)):
