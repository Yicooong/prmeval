...
def _build_video_paths(
    ...
    rel_path = os.path.join(shard_dir, f"episode_{episode_id}", filename)
...
def _process_single_stream_sample(
    ...
    """Process one streaming sample and return zero or more HF entries."""
...
def convert_agibotworld_streaming_to_hf(
    ...
    def flush_batch():
        nonlocal entries, processed, batch_samples
    ...
    for sample in ds:
        ...
        # Ensure the episode record exists before scheduling video processing.
        try:
            get_episode_record(episode_id)
        except Exception:
            skipped_no_record += 1
            continue
    ...
    return build_standard_dataset(entries)
