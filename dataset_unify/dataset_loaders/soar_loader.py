...
def _process_episode(args):
    episode_steps, ep_idx, task, output_dir, dataset_label, max_frames, fps, img_key, quality_label = args
...
def convert_soar_dataset_to_hf(
    ...
    for split_name in ["success", "failure"]:
        ...
        if not entries:
            ds_out = build_standard_dataset([])
        else:
            ds_out = build_standard_dataset(entries)
