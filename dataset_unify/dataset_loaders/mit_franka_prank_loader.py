...
def _process_episode(args):
    ...
    episode_meta, video_dir, output_dir, dataset_label, max_frames, fps = args
...
def convert_mit_franka_prank_dataset_to_hf(
    ...
    if num_workers == 1:
        ...
        for episode in tqdm(all_episodes, desc="Processing episodes"):
            result = _process_episode((episode, str(video_dir), output_dir, dataset_name, max_frames, fps))
            if result:
