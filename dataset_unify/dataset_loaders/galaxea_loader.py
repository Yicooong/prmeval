...
def _build_galaxea_video_paths(
    ...
    rel_path = os.path.join(shard_dir, f"episode_{episode_idx:06d}", filename)
...
def _process_single_galaxea_episode(args):
    episode, ep_idx, task, output_dir, dataset_name, max_frames, fps, valid_img_keys = args
...
def convert_galaxea_dataset_to_hf(
    ...
    for ep_idx, episode in enumerate(tqdm(dataset, desc=f"Processing {rlds_name} episodes")):
        ...
        info_batch.append((ep_idx, task))

        if len(episode_batch) >= batch_size or ep_idx + 1 == len(dataset):
            if num_workers == 1:
                for args in zip(
                    episode_batch,
                    [i for (i, _) in info_batch],
                    [t for (_, t) in info_batch],
