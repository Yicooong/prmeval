...
```bash
uv run python -m dataset_unify.generate_hf_dataset --config_path=dataset_unify/configs/data_gen_configs/h2r.yaml
```
...
The loader includes a simple mapping from folder names to readable descriptions and falls back to a prettified folder name if no mapping exists. You can extend `FOLDER_TO_TASK_NAME` in `dataset_unify/dataset_loaders/h2r_loader.py`.
