...
```bash
bash dataset_unify/data_scripts/egodex/download_and_convert.sh

# or run the following manually
uv run python -m dataset_unify.generate_hf_dataset --config_path=dataset_unify/configs/data_gen_configs/egodex.yaml
```

### Option 2: Manual Configuration
```bash
uv run python -m dataset_unify.generate_hf_dataset \
    --config_path=dataset_unify/configs/data_gen_configs/egodex.yaml \
...
Edit `dataset_unify/configs/data_gen_configs/egodex.yaml`:
