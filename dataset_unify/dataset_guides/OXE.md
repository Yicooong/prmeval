...
```bash
uv run python dataset_unify/generate_hf_dataset.py --config_path=dataset_unify/configs/data_gen_configs/oxe.yaml  --dataset.dataset_name oxe_<dataset_name>
```
- Using the provided script to generate all datasets:

```bash
bash dataset_unify/data_scripts/oxe/gen_all_oxe.sh
```
...
```bash
uv run dataset_unify/generate_hf_dataset.py     
    --config_path=dataset_unify/configs/data_gen_configs/oxe.yaml \
...
Note: Additional per-dataset configs (e.g., wrist cams, multiple externals) are defined in `dataset_unify/dataset_helpers/oxe_helper.py` via `OXE_DATASET_CONFIGS`. The loader currently iterates only the list above.

### Configuration

Edit `dataset_unify/configs/data_gen_configs/oxe.yaml`:
